...
# ---------------------------------------------------------
# 1. CLIENT SETUP (Switched to OpenAI)
# ---------------------------------------------------------
# Using AsyncOpenAI is best practice for APIs (FastAPI/etc) to avoid blocking.
client = AsyncOpenAI(api_key=os.getenv("OPENAI_API_KEY"))

# ---------------------------------------------------------
# 2. QUALITY GATES (The Referee - 100% Identical to v7.1)
# ---------------------------------------------------------
def validate_quality_gates(response: QuestionResponse):
    ...
    def check_specificity(text, frame_name):
        # Rule: Must force elaboration. No Yes/No questions. [cite: 80]
    ...
    def check_tone_safety_and_framing(text, frame_name):
        ...
        # Rule: Signal-First (No Interview Mode) [cite: 91]
        ...
        # Rule: No Romantic Framing [cite: 98]
...
def build_system_prompt(req, error_feedback=""):
    ...
    1. ADVISOR FRAME (Third-Person Rule) [cite: 63-66]
    ...
    2. HYPE/FLEX FRAME (Evidence Rule) [cite: 67-70]
    ...
    3. HOT TAKE FRAME (Social Rule) [cite: 71-74]
    ...
    ### GENERAL CONSTRAINTS [cite: 88-99]:
...
async def generate_with_openai(request_data):
    # GPT-4o-mini: Fast, Cheap, Smart 
    ...
    while current_try < max_retries:
        ...
        try:
            # Generate Content using OpenAI (Force JSON Object)
            ...
            parsed_response = QuestionResponse.model_validate_json(raw_content)
